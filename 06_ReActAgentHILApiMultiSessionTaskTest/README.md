...
参考MCP项目分享地址:https://github.com/NanGePlus/MCPServerTest
了解MCP相关内容，关于HTTP+SSE传输模式的介绍
...
使用数据库客户端软件远程登陆进行可视化操作，这里使用DBeaver客户端和Redis-Insight客户端软件

## 4.2 功能测试            
进入06_ReActAgentHILApiMultiSessionTaskTest文件夹下运行脚本进行测试，支持多用户访问
首先运行 celery -A utils.tasks worker --loglevel=info 启动celery服务
